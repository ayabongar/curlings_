...
public partial class Admin_NormalUserLandingPage : IncidentTrackingPage
{
    ...
    private void BindGridView()
    {
        ...
        incidents = IncidentTrackingManager.GetOocUserAssignedIncidents(SarsUser.SID, 0);
        switch (userRole)
        {
            ...
            case "System User":
                ...
                BindGridView(userRole,incidents, roleId);
                tabTeamIncidents.Visible = false;
                break;
            case "Developer":                
                roleId = new Guid(ConfigurationManager.AppSettings["AdministratorHeadTopSecret"]);
                BindGridView(userRole, incidents, roleId);
    ...
    private void BindTeamIncidents(string userRole)
    {
        ...
        if (CurrentProc.CanWorkOnOneCase)
        {
            ...
            var teamincidents = IncidentTrackingManager.GetOocUserProcessIncidents(ProcessID);
    ...
    {
        var incidents = IncidentTrackingManager.GetOocUserAssignedIncidents(SarsUser.SID, 0);
        if (incidents != null && incidents.Any())
        {
            ...
            else
            {
                var incident = incidents.FindAll(inc => inc.ProcessId == Convert.ToInt64(ProcessID) && inc.IncidentStatusId != 1 &&
                            inc.IncidentStatusId != 6);
                var table = incident.FindAll(my =>  my.ReferenceNumber.ToLower().Contains(search.ToLower())
                        || my.IncidentStatus.ToLower().Contains(search.ToLower())
                               || ((my.Subject != null) && my.Subject.ToLower().Contains(search.ToLower()))
                           || my.CreatedBySID.ToLower().Contains(search.ToLower())
                           || my.AssignedToFullName.ToLower().Contains(search.ToLower()));
                    ...
                    if (saved > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(user))
                        {
                            try
                            {
                                ...
                                if (_userRole == null)
                                {
                                    Roles.AddUserToRole(this.Page.User.GetRole(), user);
                                }
                            }
