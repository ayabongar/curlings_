    ...
    private void SaveIncident()
    {
        ...
        try
        {
            ...
            if (CurrentIncidentDetails.IncidentStatusId == 1)
            {
                ...
                if (!string.IsNullOrEmpty(user))
                {
                    try
                    {
                        ...
                        {
                            Roles.AddUserToRole("System User", user);
                            //if (!drpRoles.SelectedItem.Text.Contains("System User"))
                            // Roles.AddUserToRole(drpRoles.SelectedItem.Text, user);
                        ...
                        //else
                        //{
                        //    if (drpRoles.SelectedItem.Text != _userRole[0].Description)
                        //        Roles.AddUserToRole(drpRoles.SelectedItem.Text, user);
                        //    else
                        //        Roles.AddUserToRole("System User", user);
                        //}
            ...
            else
            {
                ...
                if (drpStatuses.SelectedItem.Text.Equals("Complete"))
                {
                    UpdateIncidentStatuses("4");
                }               
            }
            ...
            case "Submit":
                {
                    ...
                    if (CurrentIncidentDetails.DueDate < DateTime.Today && (CurrentIncidentDetails.IncidentStatusId != 4 || CurrentIncidentDetails.IncidentStatusId != 5 || CurrentIncidentDetails.SLADate != null))
                    {
                        MessageBox.Show("Date SLA date  and reason for updating SLA Date is a required field.");
                        return;
                    }
            ...
            case "Print":
                {
                    ...
                    //    IncidentID));
                    var page = "~/OOC/Admin/RegisterUserIncident";
                    Response.Redirect(String.Format("~/OOC/Admin/CoverPage.aspx?procId={0}&incId={1}&msgId=1&pg={2}", ProcessID, IncidentID, page));
                    break;
                }
