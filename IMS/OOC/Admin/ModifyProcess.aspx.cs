...
public partial class ModifyProcessPage : IncidentTrackingPage
{
    ...
    {
        var processes = IncidentTrackingManager.GetIncidentProcesses(SarsUser.SID).Where(p => p.ProcessId== OocInternalProcessId || 
        p.ProcessId==OocExternalProcessId || p.ProcessId== TaxEscalationProcessId).ToList();
    ...
    protected void SubmitProcessData(object sender, EventArgs e)
    {
        ...
        var saved = IncidentTrackingManager.SaveProcess(processId, name, chkActive.Checked, txtPrefix.Text, Convert.ToString(Math.Floor(maFileSize)),ckAddCoverPage.Checked,chkCreater.Checked,ckCanShareIncidents.Checked);
        IncidentTrackingManager.SaveAuditTrail(SarsUser.SID, 
                                     string.Format(
