...
public partial class Admin_RegisterUserIncident :IncidentTrackingPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (oocOffice.Length == 2)
        {
            foreach (var item in oocOffice)
            {
                if (item.Trim().Equals(ProcessID))
                {
        ...
        if (!IsPostBack)
        {
            ...
            if (CurrentProcessDetails.ProcessId == int.Parse(System.Configuration.ConfigurationManager.AppSettings["Legislative"]))
            {
                drpStatuses.Bind(status.FindAll(st => st.IncidentStatusId > 3 && st.IncidentStatusId != 6 && !st.Description.Contains("Deleted") && st.IncidentStatusId != 4 && st.IncidentStatusId != 5),
                    "Description", "IncidentStatusId");
                tdAssignedTo.InnerText = "Senior manager/Line manager/Legal manager:";
            }
            else
            {
                if (!CurrentProcess.ReAssignToCreater)
                {
                    drpStatuses.Bind(status.FindAll(st => st.IncidentStatusId > 3 && st.IncidentStatusId != 6 && !st.Description.Contains("Deleted")
                        && !st.Description.Contains("Finalised") && !st.Description.Contains("Return to originator")), "Description", "IncidentStatusId");
                }
                else
                {
                    drpStatuses.Bind(status.FindAll(st => st.IncidentStatusId > 3 && st.IncidentStatusId != 6 && !st.Description.Contains("Deleted")
                       && !st.Description.Contains("Close")  && !st.Description.Contains("Finalised") && !st.Description.Contains("Return to originator")), "Description", "IncidentStatusId");
    ...
    protected void Toolbar1_ButtonClicked(object sender, SCS.Web.UI.WebControls.ButtonEventArgs e)
    {
        switch (e.CommandName)
        {
            case "Submit":
            {
                ...
                if (CurrentProcessDetails.ProcessId ==
                    int.Parse(System.Configuration.ConfigurationManager.AppSettings["Legislative"]))
                {
                    if (string.IsNullOrEmpty(UserSelector1.SID))
                    {
                        MessageBox.Show("Senior manager/ Line manager / Legal manager is Required.");
                        return;
                    }
                }
                else if (string.IsNullOrEmpty(UserSelector1.SID))
                {
        ...
        if (Request.PhysicalApplicationPath != null)
        {
            ...
            if (CurrentIncidentDetails.DueDate != null)
            {
                var body = string.Format(tempate,
                    ...
                    {
                        var sendToUser = SarsUser.GetADUser(UserSelector1.SelectedAdUserDetails.SID);
