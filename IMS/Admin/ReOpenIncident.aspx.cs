            ...
            case "Submit":
                {
                    ...
                    if (CurrentProc.ProcessId == int.Parse(System.Configuration.ConfigurationManager.AppSettings["Legislative"]))
                    {
                        ...
                        {
                            UserSelector2.Focus();
                            MessageBox.Show("Please select Senior manager/Line manager/Legal manager");
                            return;
                        }
                        var saved = IncidentTrackingManager.ReOpenIncident(IncidentID);
                           saved = IncidentTrackingManager.ChangeAssignee(IncidentID, UserSelector2.SelectedAdUserDetails.SID.Trim());
                        if (saved > 0)
                        {
                            AddNote();
                            ...
                            //MessageBox.Show(string.Format("Incident was re-assigned  to the Line Manager."));
                        }
                    }
                    else
                    {
