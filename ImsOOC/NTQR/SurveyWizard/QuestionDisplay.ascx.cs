...
public partial class NTQR_SurveyWizard_QuestionDisplay : IncidentTrackingControl
{
    ...
    public ASP.ntqr_surveywizard_matrixquestion_ascx MatrixQuestion
    {
        get { return matrix; }
    }

    public ASP.ntqr_surveywizard_hierarchicallookup_ascx HierarchicalLookup
    {
    ...
    protected void ddlQuestionAnswers_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlQuestionAnswers.SelectedIndex > 0)
        {
            ...
            // this apply to the Legislative process Only.
            var currProcessId = CurrentProcess.ProcessId;
            if (currProcessId == int.Parse(System.Configuration.ConfigurationManager.AppSettings["Legislative"]))
            {
                if (ddlQuestionAnswers.SelectedItem.Text.ToLower().Contains("supported") ||
                    ddlQuestionAnswers.SelectedItem.Text.ToLower().Contains("not supported") ||
                    ddlQuestionAnswers.SelectedItem.Text.ToLower().Contains("qualified support"))
                {
                    IncidentTrackingManager.AssignLegislativeOffice(CurrentIncident.IncidentID, 1, currProcessId);
                }
                else
                {
                    IncidentTrackingManager.AssignLegislativeOffice(CurrentIncident.IncidentID, 0, currProcessId);
                }
            }
        }
        else
