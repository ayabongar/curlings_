...
public partial class PrOoc_RegisterExternalIncident : IncidentTrackingPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                {
                    CreateProcessField(Request.QueryString["procId"]);
                }
            }
        }
    }
    private void PesistObjectToPage()
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                GovernmentEntity.Visible = Entity.SelectedItem.Text.Contains("SA Government") ? true : false;
                tdGovernmentEntity.Visible = Entity.SelectedItem.Text.Contains("SA Government") ? true : false;
                lblStatekHolder.Visible = Entity.SelectedItem.Text.Contains("Stake") ? true : false;
                StakeHolder.Visible = Entity.SelectedItem.Text.Contains("Stake") ? true : false;
                lblStatekHolder.InnerText = @"If " + Entity.SelectedItem.Text + @" Specify:";
    ...
    protected void Toolbar1_ButtonClicked(object sender, SCS.Web.UI.WebControls.ButtonEventArgs e)
    {
        ...
        var controls =
            this.Master.FindControl("MainContent")
                .FindControl("tbContainer")
                .FindControl("tbDetails")
                .FindControl("UpdatePanel1") as Control;
        switch (e.CommandName)
        {
            case "Submit":
                {
                    ...
                    //    DueDate.Focus();
                    //    MessageBox.Show(
                    //        string.Format("Incident Due Date must be greater than Incident Registration Date "));
                    //    return;
                    //}

                    SaveAnswers();

                    Response.Redirect(String.Format("~/PrOoc/RegisterExternalIncident.aspx?procId={0}&incId={1}&type={2}",
                         CurrentIncidentDetails.ProcessId, CurrentIncidentDetails.IncidentID, "External"));
            ...
            case "Print":
                {
                    var page = "~/PrOoc/RegisterExternalIncident";
                    Response.Redirect(String.Format("~/Admin/CoverPage.aspx?procId={0}&incId={1}&msgId=1&pg={2}", ProcessID,
                        IncidentID, page));
                    break;
                }
            case "AcknowledgementLetter":
                {
                    var page = "~/PrOoc/RegisterExternalIncident";
                    Response.Redirect(String.Format("~/Admin/Acknowledgement.aspx?procId={0}&incId={1}&msgId=1&pg={2}",
                        ProcessID, IncidentID, page));
                    break;
                }
            case "ReAssign":
                {
    ...
    protected void Entity_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (Entity.SelectedItem.Text.Contains("SA Government"))
        {
            GovernmentEntity.Visible = true;
            tdGovernmentEntity.Visible = true;
            lblStatekHolder.Visible = false;
            StakeHolder.Visible = false;
        }
        else if (Entity.SelectedItem.Text.Contains("Stake"))
        {
            lblStatekHolder.Visible = true;
            StakeHolder.Visible = true;
            GovernmentEntity.Visible = false;
            tdGovernmentEntity.Visible = false;
            lblStatekHolder.InnerText = @"If " + Entity.SelectedItem.Text + @" Specify:";
        }
        else
        {
            ...
            StakeHolder.Visible = false;
            GovernmentEntity.Visible = false;
            GovernmentEntity.SelectedIndex = -1;
            tdGovernmentEntity.Visible = false;
        }
    }
        ...
        if (CurrentIncidentDetails != null)
        {
            if (IncidentTrackingManager.AddWorkInfo(workInfo) > 0)
            {
                ...
                if (CurrentIncident == null)
                {
                    txtNotes.Clear();
                    Response.Redirect(
                        String.Format("~/PrOoc/RegisterExternalIncident.aspx?procId={0}&incId={1}&type={2}",
                            CurrentIncidentDetails.ProcessId, CurrentIncidentDetails.IncidentID, "External"));
                }
                else
                {
    ...
    private void SendFirstNotification(Incident cIncident)
    {
        try
        {
            ...
            var incidentURL = string.Format("http://{0}/ims/PrOoc/RegisterExternalIncident.aspx?{1}",
            ...
            if (Request.PhysicalApplicationPath != null)
            {
                ...
                if (cIncident.DueDate != null)
                {
                    var body = string.Format(tempate,
                                             ...
                                             incidentURL,
                                             string.Format("{0} | {1}", SarsUser.GetADUser(cIncident.CreatedBySID).FullName, SarsUser.GetADUser(cIncident.CreatedBySID).SID)
                        );
